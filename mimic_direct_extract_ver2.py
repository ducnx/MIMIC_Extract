"""
Generate data without hourly grouping
"""
...
def add_outcome_indicators(out_gb):
    out_gb.to_csv('/home/dxng/codes/MIMIC_Extract/out_gb.csv')
    ...
    #  'hours_in': hours, 'on': on_vals
    }, index=[0])  # icustay_id': icustay_id})
...
def continuous_outcome_processing(out_data, data, icustay_timediff):
    ...
    out_data['starttime'] = out_data.starttime.apply(lambda x: x.days*24 + x.seconds/60./60)
    out_data['endtime'] = out_data['endtime'] - out_data['intime']
    out_data['endtime'] = out_data.endtime.apply(lambda x: x.days*24 + x.seconds/60./60)
...
def standardize_units(X, name_col='itemid', unit_col='valueuom', value_col='value', inplace=True):
    ...
    def name_filter(n): return name_col_vals.contains(n, case=False, na=False)
    def unit_filter(n): return unit_col_vals.contains(n, case=False, na=False)
...
def save_numerics(
    ...
    def to_hours(x):
        # return max(0, x.days*24 + x.seconds // 3600)
        return max(0, x.days * 86400 + x.seconds)
        # return max(0, x.days * 24 + x.seconds / 60. / 60)
    ...
    X = X.groupby(ID_COLS + group_item_cols + ['hours_in']).mean()
    # tmp = X[:1000]
    # tmp.to_csv('/home/dxng/codes/MIMIC_Extract/tmp1.csv')
    # print('X')
    # print(X.columns)
    # print(X.head(40))
    # X.columns = X.columns.droplevel(0)
    # print('X.columns.names', X.columns.names)
    X.columns.names = ['Aggregation Function']
    data['max_hours'] = (data['outtime'] - data['intime']).apply(to_hours)
    data['max_hours'] = data['max_hours'] / 60. / 60
    # print('data head')
    # print(data.head(20))
    # data.to_csv('/home/dxng/codes/MIMIC_Extract/data_pop.csv')
    hours = X.reset_index(
    )[ID_COLS + ['hours_in']].sort_values(
        by=['subject_id', 'hadm_id', 'icustay_id', 'hours_in'], ascending=[True, True, True, True]
    ).drop_duplicates().set_index(ID_COLS + ['hours_in'])
    # hours.to_csv('/home/dxng/codes/MIMIC_Extract/hours.csv')
    # print('hours.columns', hours.columns)
    # print('hours.index', hours.index)
    # print(hours.head(100))
    ...
    # print('missing_hours_fill')
    # print(missing_hours_fill.head(20))
    ...
    # fill_df = data.reset_index().set_index(ID_COLS).join(hours)
    # fill_df.set_index(ID_COLS + ['hours_in'], inplace=True)
    # print('fill_df')
    # print(fill_df.head(20))
    # print(fill_df.columns)
    # Pivot table drops NaN columns so you lose any uniformly NaN.
    X = X.unstack(level=group_item_cols)
    X.columns = X.columns.reorder_levels(order=group_item_cols + ['Aggregation Function'])
    # print('X')
    # print(X.head(30))
    X = X.reindex(hours.index)
    # X.to_csv('/home/dxng/codes/MIMIC_Extract/X.csv')
    X = X.reset_index()
    X['hours_in'] = X['hours_in'] / 60. / 60
    X.set_index(ID_COLS + ['hours_in'], inplace=True)
    # tmp = X.reset_index()[:1000]
    # tmp.to_csv('/home/dxng/codes/MIMIC_Extract/tmp2.csv')
    # print('X')
    # print(X.head(100))
    # if group_by_level2:
...
def save_outcome(
    ...
    icustay_timediff = pd.Series([timediff.days*24 + timediff.seconds/60.60
    ...
    #               axis=0)
    Y = pd.concat([
        vent_data[['subject_id', 'hadm_id', 'icustay_id']],
        out_data[['subject_id', 'hadm_id', 'icustay_id']]
    ], axis=0)
    ...
    for task in tasks:
        ...
        # )
        Y = Y.merge(
            new_data[['subject_id', 'hadm_id', 'icustay_id']],
    ...
    # Y.hours_in = Y.hours_in.astype(float)
...
if __name__ == '__main__':
    ...
    if args['pop_size'] > 0:
        ...
        dynamic_hd5_filt_filename = splitext(dynamic_hd5_filt_filename)[
            0] + '_' + pop_size + splitext(dynamic_hd5_filt_filename)[1]
    ...
    if X is None:
        print("SKIPPED vitals_data")
    else:
        print("LOADED vitals_data")
    ...
    if X is not None:
        print("Numerics", X.shape, X.index.names, X.columns.names)
    if Y is not None:
        print("Outcomes", Y.shape, Y.index.names, Y.columns.names, Y.columns)
    if C is not None:
        print("Codes", C.shape, C.index.names, C.columns.names)
    if N is not None:
        print("Notes", N.shape, N.index.names, N.columns.names)
